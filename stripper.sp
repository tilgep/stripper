...
public Plugin myinfo =
{
	...
	author		= "tilgep, Stripper:Source by BAILOPAN, Lerrdy",
...
public Action Command_Stripper(int client, int args) {
	if (g_bConfigLoaded)
		ReplyToCommand(client, "The current map has a loaded stripper config (%s)", file);
	else
		ReplyToCommand(client, "The current map did not load a stripper config (%s)", file);
...
public Action Command_Dump(int client, int args)
{
	...
	FormatEx(buf2, PLATFORM_MAX_PATH, "addons/stripper/dumps");
...
public void OnMapInit(const char[] mapName)
{
	...
	FormatEx(file, sizeof(file), "addons/stripper/global_filters.cfg");
	...
	Format(file, sizeof(file), "addons/stripper/maps/%s.cfg", file);
